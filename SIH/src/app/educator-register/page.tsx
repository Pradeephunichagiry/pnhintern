  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const registerResponse = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ name, email, password, role: 'educator' }),
      });
      const registerData = await registerResponse.json();

      if (!registerResponse.ok) {
        let errorMessage = registerData.error || "An unexpected error occurred.";
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100 dark:bg-gray-900 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="text-2xl font-headline">Create Educator Account</CardTitle>
          <CardDescription>Join EcoSphere to inspire the next generation.</CardDescription>
        </CardHeader>
        <CardContent>
